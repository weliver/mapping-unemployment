...
var settings = {
  ...
  var key = {
    ...
    xPos: function(val) {
      var w = settings.width - this.width;
      var a = this.cellWidth() * (this.data.indexOf(val)+1);
      var f = w-a*(-1) - this.width;
      if (f) {
        return f;
      }
    }
  ...
  //draw the key color boxes in the bottom right  
    key.group.selectAll('rect')
      ...
      .enter()
      .append('rect')
      .attr('x', function(d) {return (key.xPos(d));})
      .attr('y', '15%')
      .attr('width', function() {return key.cellWidth();})
      .attr('height', '30')
  ...
  //draw the text inside the key boxes in the bottom right
    key.group.selectAll('text')
      .data(key.data)
      .enter().append('text')
      .attr('x', function(d) {return key.xPos(d)+10;})
      .attr('y', '75%')
      .style('color', 'black')
